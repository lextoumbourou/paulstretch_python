...
import sys
from numpy import (
    int16, linspace, tile, fft, random, pi, exp, zeros, append, floor)
...
def paulstretch(samplerate, smp, stretch, windowsize_seconds, outfilename):
    ...
    while True:
        ...
        # randomize the phases by multiplication with a
        # random complex number with modulus = 1
...
if (len(args) < 2 or options.stretch <= 0.0 or options.window_size <= 0.001):
    print (
        ('Error in command line parameters. '
         'Run this program with --help for help.')
    )
    sys.exit(1)
